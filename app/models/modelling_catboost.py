"""
CatBoost Model Training for Stock Price Prediction

This script trains a CatBoost classifier to predict whether SPY will go up or down
...
PHASE 1: Train with all features (115)
  ...
  4. Train CatBoost model with default hyperparameters
...
PHASE 2: Retrain with selected features (40)
  ...
  9. Retrain CatBoost model with same hyperparameters
  10. Evaluate and compare performance vs full model
  11. Save both models, feature lists, and visualizations

Hyperparameter Strategy:
-----------------------
Model uses CatBoost default parameters for initial baseline:
...
- use_best_model=True (automatically use best iteration)

CatBoost Advantages:
-------------------
- Handles categorical features natively (though we have none)
- Ordered boosting reduces overfitting
- Symmetric trees for faster inference
- Built-in handling of feature interactions

Output Files (saved to output/models/catboost/):
----------------------------------------------
...
- feature_importance_top30.png - CatBoost built-in importance plot
...
the model performance will be evaluated against the baseline. CatBoost's ordered
boosting and symmetric trees may provide different generalization compared to
...
OUTPUT_DIR = 'output/models/catboost'
TRAIN_END_DATE = '2024-12-31'
TEST_START_DATE = '2025-01-01'
RANDOM_STATE = 42

# CatBoost hyperparameters - Using defaults for baseline
CATBOOST_PARAMS = {
    # Core objective and evaluation
    'loss_function': 'Logloss',  # Binary classification
    'eval_metric': 'AUC',  # Use AUC for evaluation and early stopping
    'custom_metric': ['AUC', 'Accuracy'],  # Track multiple metrics
    
    # Tree structure parameters (CatBoost defaults)
    'depth': 6,  # Maximum depth of trees
    'iterations': 1000,  # Maximum number of trees (early stopping will determine actual)
...
def train_catboost(X_train, y_train, X_test, y_test, params):
    ...
    print(f"\nTraining with up to {params['iterations']} iterations...")
    print(f"Early stopping: {params['od_wait']} rounds")
    ...
    model.fit(
        train_pool,
        eval_set=test_pool,
        plot=False  # Disable plotting during training
    )
    ...
    evals_result = {
        'train': model.get_evals_result()['learn'],
        'test': model.get_evals_result()['validation']
    }
...
def plot_feature_importance(model, feature_names, top_n=30, output_dir='output/models'):
    ...
    plt.title(f'Top {top_n} Features by Importance (CatBoost)')
...
def calculate_permutation_importance(model, X_test, y_test, feature_names, top_n=40, output_dir='output/models'):
    ...
    plt.title(f'Top {top_n} Features by Permutation Importance (CatBoost)')
    plt.gca().invert_yaxis()
    plt.tight_layout()
...
def plot_training_curves(evals_result, output_dir='output/models'):
    ...
    # CatBoost returns metric keys in different format - check what's available
    train_metrics = evals_result['train']
    test_metrics = evals_result['test']
    
    # Try to get AUC metric (could be 'AUC' or other variations)
    metric_key = 'AUC' if 'AUC' in train_metrics else list(train_metrics.keys())[0]
    
    train_metric = train_metrics[metric_key]
    test_metric = test_metrics[metric_key]
    ...
    plt.ylabel('AUC')
    plt.title('CatBoost Training Curves (AUC)')
    plt.legend()
...
def save_model_and_results(model, metrics, output_dir='output/models'):
    ...
    model_path = os.path.join(output_dir, 'catboost_model.json')
...
def main():
    ...
    """
    print("="*60)
    print("CATBOOST STOCK PREDICTION MODEL")
    print("="*60)
    ...
    model_full, evals_result_full = train_catboost(
        X_train_clean, y_train_clean,
        X_test_clean, y_test_clean,
        CATBOOST_PARAMS.copy()
    )
    ...
    # 6. Feature importance (CatBoost built-in)
    ...
    model_selected, evals_result_selected = train_catboost(
    ...
    model_path = os.path.join(OUTPUT_DIR, 'catboost_model_selected_features.json')
    model_selected.save_model(model_path, format='json')
    ...
    print("  - catboost_model.json (full model)")
    print("  - catboost_model_selected_features.json (selected features model)")
